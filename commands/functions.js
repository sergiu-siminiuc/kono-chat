const talkedRecently = new Set();
	...
	setTimeout(() => {
	  ...
	  if (score.points == 25) {
	    role = message.guild.roles.find("name", client.config.defaultSettings.memberRole)
		  message.member.addRole(role).catch(console.log)
		  message.reply(`You have been assigned the role: ${client.config.defaultSettings.memberRole}`)
	  }
	  	};
	  ...
	  client.permlevel = message => {
	    ...
	    try {
	      const modRole = message.guild.roles.find(r => r.name.toLowerCase() === client.config.defaultSettings.modRole.toLowerCase());
	      if (modRole && message.member.roles.has(modRole.id)) permlvl = 2;
	    } catch (e) {
	      console.warn("modRole not present in guild settings. Skipping Moderator (level 2) check");
	    }
	    try {
	      const adminRole = message.guild.roles.find(r => r.name.toLowerCase() === client.config.defaultSettings.adminRole.toLowerCase());
